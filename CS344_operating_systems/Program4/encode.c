...
void validateInput(INPUT* p, INPUT* k)
{
	int valid = 1;
	//check length
	if (p->length > k->length) { valid = 0; fprintf(stderr, "CLIENT: Key is too short\n"); exit(1); }
...
{
	if (strcmp(e->message, "NO&&") == 0) { fprintf(stderr, "CLIENT: Server rejected connection\n"); exit(1); }
}
