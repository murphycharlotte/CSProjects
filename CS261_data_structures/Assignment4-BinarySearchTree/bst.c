...
* Date: 10/29/2016
* Description: CS 261_400 Assignment 4 - Binary Search Tree
*/
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	//if val is greater than or equal to
	else if (comp > -1)
	{
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	int comp = 5; //initialize to arbitrary int besides 0, 1, -1 

	while ((nodePtr != NULL) && (comp != 0)) {
		comp = compare(val, nodePtr->val);

		if (comp == 0)	//if value is found, return 1
		{
			return 1;
		}
		else if (comp == -1) //if value is LT, look left
		{
			nodePtr = nodePtr->left;
		}
		else if (comp == 1) //if value is GT, look right
		{
			nodePtr = nodePtr->right;
		}
	}

	return 0;	//if value is not found, return 0
}
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	struct Node* nodePrt = cur;
	
	int comp = compare(val, cur->val);		//int to store compare() results

	/*if cur == val, free cur and return pointer to appropriate child, if any*/
	if (comp == 0)							//
	{
	...
	/*If val is less than cur, look left*/
	else if (comp == -1)
	{
		cur->left = _removeNode(cur->left, val);
	}
	/*If val is greater than cur, look left*/
	else if (comp == 1)
	{
...
void testAddNode() {
	...
	//check the root node
	if (compare(tree->root->val, (struct data*)&myData1) != 0) {
    ...
    if (compare(tree->root->left->val, (struct data*)&myData2) != 0) {
		printf("TESTaddNode compare 2 worked\n");
    ...
    if (compare(tree->root->right->val, (struct data*)&myData3) != 0) {
    ...
    if (compare(tree->root->left->left->val, (struct data*)&myData4) != 0) {
