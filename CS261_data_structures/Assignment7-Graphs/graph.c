...
{
    // FIXME: Implement
	//add to top of deque, remove from top of deque
	assert(graph != NULL);
	assert(source != NULL);
	assert(destination != NULL);
	Deque* container = dequeNew();
	Vertex* v;
	Vertex **reachable = malloc(sizeof(Vertex) * graph->numVertices);
	...
	int qtyReachable = 0;
	dequePushFront(container, source);	//add source to queue
	while (container->size > 0) {
		v = dequeFront(container);		//next vertex to check
		if (EQ(v->label, destination->label)) {		//return 1 if vertex == destination
			dequeDelete(container);
			free(reachable);
			return 1;
		}
		else {
			int vFound = 0;
			for (int i = 0; i < qtyReachable; i++) {	//look in reachable for v
				if (EQ(reachable[i]->label, v->label)) {
					vFound = 1;
					i = qtyReachable;
				}
			}

			if (EQ(vFound, 0)) {	//if v in not in reachable, add v
				reachable[qtyReachable] = v;
				qtyReachable++;
			}

			v->isVisited = 1;				//mark vertex as visited

			dequePopFront(container);		//remove first element, the element just examined, from queue container

			for (int i = 0; i < v->numNeighbors; i++) {		//add unvisited neighbors to queue container
				if (EQ(v->neighbors[i]->isVisited, 0)) {
					dequePushFront(container, v->neighbors[i]);
				}
			}
		}
	}
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	int qtyReachable = 0;
	dequePushBack(container, source);					//add source to queue
	...
	while (container->size > 0) {
		...
		if (EQ(v->label, destination->label)) {			//return 1 if vertex == destination
			...
			if (EQ(vFound, 0)) {						//if not, found, add v to reachable
				reachable[qtyReachable] = v;
				qtyReachable++;
			}
			...
			for (int i = 0; i < v->numNeighbors; i++) {		//add unvisited neighbors to queue container
				if (EQ(v->neighbors[i]->isVisited, 0)) {
					dequePushBack(container, v->neighbors[i]);
				}
			}
			dequePopFront(container);		//remove first element, the element just examined, from queue container
		}
